...
export const Profile = () => {
  ...
  const PanelGroup = ({ title, children }: { title: string, children: React.ReactNode }) => (
    ...
    >
      ...
      <div className="clay-panel bg-white/40 backdrop-blur-md border border-white/60 rounded-[20px] p-2 flex flex-col gap-1 shadow-sm shadow-[#5BA8A0]/5">
        {children}
      </div>
    </motion.div>
  );
    ...
    <div className="flex-1 overflow-y-auto w-full pb-20 relative z-10 no-scrollbar">
      <div className="max-w-2xl mx-auto px-6 md:px-12 py-10 md:py-16">
          ...
          <motion.div 
            ...
            className="clay-panel bg-white/60 backdrop-blur-md border border-white/60 p-6 md:p-8 rounded-[24px] mb-[40px] shadow-sm shadow-[#5BA8A0]/5 flex justify-between items-center"
          >
          ...
          <AnimatePresence>
            {editingField === "name" && (
              ...
              >
                <div className="clay-panel bg-white/90 border border-[#5BA8A0]/30 p-6 rounded-[20px] shadow-lg flex flex-col gap-3">
                  ...
                  <div className="flex items-center justify-end gap-3">
                    ...
                    <button onClick={handleSaveName} className="bg-[#5BA8A0] text-white text-[13px] font-medium px-5 py-2 rounded-xl hover:bg-[#4E938C] transition-colors shadow-sm">Save Changes</button>
                  </div>
                </div>
              </motion.div>
            )}
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      currentTheme === t.color 
                        ? "border-white scale-110 shadow-md ring-2 ring-[#5BA8A0]/40" 
                    ...
                    >
                      <div className="flex items-center gap-2 text-[13px] text-slate-600 bg-white/40 px-3 py-1.5 rounded-lg border border-white/60 shadow-sm">
                        ...
                        <input 
                          ...
                          className="bg-transparent text-teal-800 font-medium focus:outline-none px-1"
                        />
                        <span>on</span>
                      </div>
                          ...
                          return (
                            <button
                              ...
                              className={cn(
                                ...
                                isSelected 
                                  ? "bg-[#5BA8A0] text-white border-[#5BA8A0] shadow-sm" 
          ...
          <PanelGroup title="Privacy & Data">
            <Row label="Download my data">
              {exportMessage ? (
                <motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-[13px] text-teal-700 italic font-medium" style={{ fontFamily: 'var(--font-serif)' }}>
                  {exportMessage}
                </motion.span>
              ) : (
                <button onClick={handleExport} className="text-[13px] text-slate-500 hover:text-teal-700 hover:underline hover:underline-offset-2 transition-colors duration-150">
                  Request download
                </button>
              )}
            </Row>
                ...
                {editingField === "password" ? (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-end gap-3 mt-3 w-full">
                      ...
                      <button onClick={handleSavePassword} className="bg-[#5BA8A0] text-white text-[13px] font-medium px-4 py-2 rounded-xl hover:bg-[#4E938C] shadow-sm transition-all">Update password</button>
                    </div>
                    {pwdMsg && <span className="text-[12px] text-teal-600 mt-1">{pwdMsg}</span>}
                  </motion.div>
                ) : (
                  <button onClick={() => handleEdit("password")} className="text-[13px] text-slate-500 hover:text-teal-700 hover:underline hover:underline-offset-2 transition-colors duration-150">Change password</button>
                )}
              </AnimatePresence>
            </Row>
